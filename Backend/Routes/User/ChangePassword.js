...
router.put('/change-password', authenticate, [
  ...
  body('newPassword').isLength({ min: 8 }).withMessage('Minimum length of password must be 8')
    ...
    .not().isLowercase().withMessage('Password must contain an uppercase letter')
], async (req, res) => {
  ...
  try {
    ...
    const updatedUser = await User.findByIdAndUpdate(userId, {password: hash}, {new: true}).select('-password');
    ...
    if(error) {
      ...
      res.status(500).json({ error: `Error occured while updating password: ${error.message}`})
    }
  }
})
